...
DT_CONTRACT_COLUMNS = [
    ...
    ("COMUNA_CELEBRACION", lambda c: c._steps_contract_payload().get("signature_commune_code") or ""),
    ...
    ("FAENA_COMUNA", lambda c: c._steps_contract_payload().get("work_commune_code") or ""),
    ...
    ("REM_AFP", lambda c: c._steps_pension_payload().get("afp_name") or ""),
    ("REM_SALUD", lambda c: c._steps_pension_payload().get("health_name") or "FONASA"),
    ("REM_OTROS_PACTOS", lambda c: ""),
    ("TIPO_JORNADA", lambda c: c._steps_contract_payload().get("tipo_jornada_code") or ""),
