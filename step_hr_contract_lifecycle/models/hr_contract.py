...
class HrContract(models.Model):
    """Amplía hr.contract sólo con datos genéricos, independientes del
    motor de nómina instalado (department_id, job_id, resource_calendar_id,
    wage ya existen en hr.contract y se reutilizan tal cual).

    El núcleo NUNCA referencia campos propios de un motor específico
    (afp_id, isapre_id, causal_id, step.fundo, etc.): eso se resuelve
    exclusivamente a través de los métodos ``_steps_*_payload`` que cada
    adaptador (agrícola / SimpleDigital) sobreescribe."""
    ...
    work_location_id = fields.Many2one(
        "hr.work.location",
        string="Ubicación contractual",
        tracking=True,
        help="Concepto estándar de Odoo. El adaptador agrícola sincroniza "
        "aquí el fundo/predio elegido; el núcleo no depende de step.fundo.",
    )
    ...
    # El núcleo nunca lee afp_id/isapre_id/causal_id/step.fundo ni ningún otro
    # campo propio de un motor de nómina específico: todo pasa por estos tres
    # métodos, que cada adaptador (agrícola / SimpleDigital) sobreescribe con
    # su mapeo real. Los generadores DT y el motor de finiquito consumen
    # exclusivamente estos payloads normalizados.

    def _steps_contract_payload(self):
        """Datos contractuales normalizados (ubicación, jornada, etc.).

        El núcleo da un valor por defecto razonable usando sus propios
        campos genéricos (res.city vía commune_signature_id/
        work_commune_id), suficiente para el adaptador agrícola. El
        adaptador SimpleDigital sobreescribe con su propia fuente
        (hr.employee.hr_commune, res.country.commune)."""
        self.ensure_one()
        return {
            "signature_commune_code": self.commune_signature_id.code or "",
            "work_commune_code": self.work_commune_id.code or "",
            "job_name": self.job_id.name or "",
            "tipo_jornada_code": "",
        }

    def _steps_pension_payload(self):
        """Datos previsionales normalizados (AFP, salud)."""
        self.ensure_one()
        return {
            "afp_name": "",
            "health_name": "",
    ...
    def _steps_termination_payload(self):
        """Causal de término normalizada como registro de
        ``step.hr.termination.cause`` (o vacío si el contrato no tiene
        término asociado todavía)."""
        self.ensure_one()
        return {"termination_cause_id": self.env["step.hr.termination.cause"].browse()}
