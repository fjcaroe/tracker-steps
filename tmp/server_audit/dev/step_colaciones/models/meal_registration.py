...
class StepColacionRegistration(models.Model):
    ...
    def get_dashboard_data(self):
        ...
        today_domain = base_domain + [("meal_date", "=", today)]
        entered = self.search_count(today_domain + [("state", "=", "entered")])
        valued = self.search(today_domain + [("state", "in", ("validated", "costed", "accounted"))])
        total_today = self.search_count(today_domain + [("state", "!=", "cancelled")])
        offline_today = self.search_count(today_domain + [("source", "=", "totem_offline")])
        unique_employees = len(set(self.search(today_domain + [("state", "!=", "cancelled")]).mapped("employee_id").ids))
        by_product = self._read_group(
            today_domain + [("state", "!=", "cancelled")],
            ["product_tmpl_id"], ["__count"], order="__count desc", limit=5,
        )
        recent = self.search(base_domain, limit=6, order="event_datetime desc")
        return {
            "date": fields.Date.to_string(today),
            "total_today": total_today,
            "entered_today": entered,
            "validated_today": len(valued),
            "costed_today": self.search_count(today_domain + [("state", "=", "costed")]),
            "accounted_today": self.search_count(today_domain + [("state", "=", "accounted")]),
