...
class ReportByProcessWizard(models.TransientModel):
    _name = 'report.by.process.wizard'
    _description = 'Reporte de progreso xls'
    _check_company_auto = True

    def _compute_file_name(self):
        for rec in self:
            rec.file_name = 'reporte_por_proceso.xlsx'

    # Resultado
    file = fields.Binary(string='Archivo')
    file_name = fields.Char(string='Nombre', compute='_compute_file_name')
    employee_ids = fields.Many2many(
        'hr.employee', 'employee_process_rel', 'employee_id', 'wizard_id', string='Empleados')
    ...
    def generate_report(self):
        ...
        #     worksheet.write_string(row, 11, str(tarja.tarja_id.date) or '', numerico)
        ...
        #     worksheet.write_string(row, 20, tarja.employee_id.pin or '', numerico)
        #     worksheet.write_number(row, 21, tarja.quantity or 0, numerico)
        #     worksheet.write_number(row, 22, tarja.hrs or 0, numerico)
        #     worksheet.write_number(row, 23, tarja.hrs_extra or 0, numerico)
        #     worksheet.write_number(row, 24, tarja.hrs_total or 0, numerico)
        #     worksheet.write_number(row, 25, tarja.cant_minima or 0, numerico)
        #     worksheet.write_number(row, 26, tarja.tarifa or 0, numerico)
        #     worksheet.write_number(row, 27, tarja.total_trato or 0, numerico)
        #     worksheet.write_number(row, 28, tarja.sueldo_base or 0, numerico)
        #     worksheet.write_number(row, 29, tarja.valor_hrs_extra or 0, numerico)
        #     worksheet.write_number(row, 30, tarja.variable_trato or 0, numerico)
        #     worksheet.write_number(row, 31, tarja.sem_corrida or 0, numerico)
        #     worksheet.write_number(row, 32, tarja.sab_dom or 0, numerico)
        #     worksheet.write_number(row, 33, tarja.gratifica or 0, numerico)
        #     worksheet.write_number(row, 34, tarja.cost_sueldo or 0, numerico)
        #     worksheet.write_number(row, 35, tarja.seguro or 0, numerico)
        #     worksheet.write_number(row, 36, tarja.feriado or 0, numerico)
        #     worksheet.write_number(row, 37, tarja.ias or 0, numerico)
        #     worksheet.write_number(row, 38, tarja.cost_empresa or 0, numerico)
        #     # total += line[1]
        #     row += 1
        #     end_row = row
        for tarja in result2:
            ...
            worksheet.write_string(row, 7, tarja.cost_id.name or '', caracteres)
            worksheet.write_string(row, 8, '', numerico_cant)
            worksheet.write_string(row, 9, tarja.cost_id.variedad_id.name or '', caracteres)
            worksheet.write_string(row, 10, tarja.uom_id.name or '', caracteres)
            worksheet.write_string(row, 11, str(tarja.tarja_cost_id.date) or '', caracteres)
            ...
            worksheet.write_number(row, 21, tarja.quantity or 0, numerico_cant)
            worksheet.write_number(row, 22, tarja.hrs or 0, numerico_cant)
            worksheet.write_string(row, 23, '', caracteres)
            worksheet.write_string(row, 24, '', caracteres)
            worksheet.write_number(row, 25, tarja.cant_minima or 0, numerico_cant)
            worksheet.write_number(row, 26, tarja.tarifa or 0, numerico_price)
            worksheet.write_number(row, 27, tarja.trato_total or 0, numerico_price)
