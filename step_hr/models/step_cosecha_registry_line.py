...
class StepCosechaRegistryLine(models.Model):
    ...
    def _compute_total_hrs(self):
        ...
        for move in self:
            if move.tarja_id.state not in ['conta']:
                ...
                move.sueldo_base = ((float(move.contract_id.sueldo_base) / int(ultimo_de_mes)) / horas) * float(move.hrs)
                if move.hrs_extra > 0:
    ...
    cost_id_domain = fields.Many2many('account.analytic.account', 'registry_cost_rel', 'cost_id',
                                      'registry_id', string='Centro costos', compute='_compute_total_hrs', store=True)
